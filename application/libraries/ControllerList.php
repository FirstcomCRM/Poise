	...
	private function setControllers() {
		...
		foreach(glob(APPPATH . 'controllers/*') as $controller) {
			...
			if(is_dir($controller)) {
				...
				if($controllername != 'login' && $controllername != 'logout' && substr($controllername, 0, 3) != 'pv_') {
					...
					if(is_array($aMethods)){
						foreach($aMethods as $method) {
							if($method != '__construct' && $method != 'get_instance' && $method != $controllername && substr($method, 0, 3) != 'aj_' && substr($method, 0, 3) != 'pv_') {
								$aUserMethods[] = $method;
							}
						}
					}
