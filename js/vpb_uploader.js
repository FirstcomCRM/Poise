...
function vpb_multiple_file_uploader(vpb_configuration_settings)
{
		...
		if(this.vpb_files.length > 0)
		{
 			...
 			for(var i = 0; i<this.vpb_files.length; i++)
			{
				...
				if(typeof this.vpb_files[i] != undefined && this.vpb_files[i].name != "")
				{
					...
					//vpb_added_files_displayer += '<tr id="add_fileID'+vpb_file_id+'" class="'+new_classc+'"><td name ="filename" id ="filename">'+this.vpb_files[i].name.substring(0, 40)+'</td><td><span id="uploading_'+vpb_file_id+'"><span style=color:blue;>Ready</span></span></td><td>'+vpb_actual_fileSize+'</td><td><span id="remove'+vpb_file_id+'"><span class="vpb_files_remove_left_inner" onclick="vpb_remove_this_file(\''+vpb_file_id+'\',\''+this.vpb_files[i].name+'\');">Remove</span></span></td></tr></div>';
					vpb_added_files_displayer += '<tr id="id-'+i+'" class="'+new_classc+'"><td><form id ="uploaded-files"><input type="text" id="filename" name="filename" value='+this.vpb_files[i].name.substring(0, 40)+'  "></input></form></td><td><span id="uploading_'+vpb_file_id+'"><span style=color:blue;>Ready</span></span></td><td>'+vpb_actual_fileSize+'</td><td><span id="remove'+vpb_file_id+'"><span class="vpb_files_remove_left_inner" onclick="vpb_remove_this_file(\''+vpb_file_id+'\',\''+this.vpb_files[i].name+'\');">Remove</span></span></td></tr></div>';
					//vpb_added_files_displayer += '<tr id="add_fileID'+vpb_file_id+'" class="'+new_classc+'"><td>'+vpb_file_icon+' <input type="text" id="filename" name="filename" value='+this.vpb_files[i].name.substring(0, 40)+' /></td><td><span id="uploading_'+vpb_file_id+'"><span style=color:blue;>Ready</span></span></td><td>'+vpb_actual_fileSize+'</td><td><span id="remove'+vpb_file_id+'"><span class="vpb_files_remove_left_inner" onclick="vpb_remove_this_file(\''+vpb_file_id+'\',\''+this.vpb_files[i].name+'\');">Remove</span></span></td></tr></div>';
					//var formdata = this.vpb_files[i].name.substring(0, 40);
					
				}
				
			}
			//Display browsed files on the screen to the user who wants to upload them
			$("#files-table").append(vpb_added_files_displayer);
			var formdata = $("#uploaded-files").serializeObject();
	...
	vpb_multiple_file_uploader.prototype.vpb_submit_added_files = function(){ var formdata = $("#uploaded-files").serializeObject();
