$( document ).ready(function() {
  ...
  $('#ico-add').click(function(e) { 
    ...
    if ( $('#description').val() == '') {
      ...
      $.ajax({
          type: "POST",
          url: burl + "delivery/aj_addDeliverydetail", 
          data: { 
            hid_delivery_id    : $('#hid-delivery-id').val(),
            no                : $('#no').val(),
          ...
          success: function(data){ 
            ...
            if(result['status'] == 'success') {
                updateTable(result['delivery_detail']);
                resetForm();
            }
            else {
              alert(result['msg']);
    ...
    $('#detail-table').on('click', '.edit-di', function(e) {
        ...
        $.ajax({
          type: "POST",
          url: burl + "delivery/aj_getDeliverydetail/" + row[1], 
          data: { },
          success: function(data){ 
            // console.log(data); 
            var result = $.parseJSON(data);
    ...
    $('#detail-table').on('click', '.delete-di', function(e) {
        ...
        if (r == true) {
          ...
          $.ajax({
              type: "POST",
              url: burl + "delivery/aj_deleteDeliverydetail/" + row[1], 
              data: { },
              success: function(data){ 
                var result = $.parseJSON(data);
                if(result['status'] == 'success') {
    ...
    $('#ico-update').click(function(e) { 
        ...
        if ( $('#description').val() == '') {
          alert("Please Enter Description");
            ...
            $.ajax({
                type: "POST",
                url: burl + "delivery/aj_updateDeliverydetail/" + editkey, 
                data: { 
                  hid_delivery_id  : $('#hid-delivery-id').val(),
                  no                : $('#no').val(),
                  description       : $('#description').val(),
    ...
    $('#btn-submit').click(function(e) {   
      ...
      if( $('#quotation-id').val() == '' ) {
        ...
        $.ajax({
          type: "POST",
          url: burl + "delivery/edit/" + $('#hid-delivery-id').val(), 
  ...
  function updateTable(arr) {
    ...
    $.each(arr, function( i, value ) {   
        var classname = 'id-' + value.delivery_detail_id ; 
